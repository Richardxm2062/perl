...
"MSWin32-x64-multi-thread-6.1.7601" or
	die "Errno architecture (MSWin32-x64-multi-thread-6.1.7601) does not match executable architecture ($Config{'archname'}-$Config{'osvers'})";
...
our %EXPORT_TAGS = (
    ...
    WINSOCK => [qw(
	WSAEADDRINUSE WSAETOOMANYREFS WSAEINPROGRESS WSAEDISCON
	WSAEHOSTUNREACH WSAENOBUFS WSAEPROTONOSUPPORT WSAEDQUOT
	WSAESOCKTNOSUPPORT WSAEINTR WSAENOPROTOOPT WSAEINVALIDPROVIDER
	WSAEREMOTE WSAECONNABORTED WSAEADDRNOTAVAIL WSAEINVAL WSAEHOSTDOWN
	WSAEFAULT WSAEWOULDBLOCK WSAEDESTADDRREQ WSAEPROVIDERFAILEDINIT
	WSAESHUTDOWN WSAENETDOWN WSAECONNREFUSED WSAENOTSOCK WSAEMFILE
	WSAENOTEMPTY WSAENETRESET WSAEPROTOTYPE WSAECONNRESET WSAEBADF
	WSAEALREADY WSAEOPNOTSUPP WSAESTALE WSAEPFNOSUPPORT WSAEUSERS WSAELOOP
	WSAEAFNOSUPPORT WSAECANCELLED WSAEINVALIDPROCTABLE WSAENOMORE
	WSAENAMETOOLONG WSAETIMEDOUT WSAEMSGSIZE WSAEISCONN WSAEREFUSED
	WSAEPROCLIM WSAEACCES WSAENETUNREACH WSAENOTCONN
    )],
);
